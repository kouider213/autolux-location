...
export async function getDashboardStats(userRole) {
  ...
  // CA total = prix_client × jours
  const totalRevenue = accepted.reduce((sum, b) => {
    const days = calcNbDays(b);
    const prixClient = getPrixClientJour(b);
    // Si final_price semble être le total (et non un prix/jour), on l'utilise tel quel
    const finalTotal = Number(b.final_price || 0);
    const isTotal = finalTotal >= prixClient * days * 0.9 && days > 1;
    return sum + (isTotal ? finalTotal : prixClient * days);
  }, 0);

  // Part Houari = prix_houari × jours
