...
function calcNbDays(b) {
  ...
  if (b.start_date && b.end_date) {
    ...
    const d = Math.round(diff / (1000 * 60 * 60 * 24)) + 1; // jours inclus (départ + retour)
    return d > 0 ? d : 1;
  }
  return 1;
}
 ...
 * TOUJOURS cars.base_price — on ignore base_price_snapshot car
 * les réservations legacy y ont stocké le total au lieu du prix/jour.
 */
...
// MESSAGE WHATSAPP
// ============================================================
export function generateWhatsAppMessage(booking, car, wa = '32466311469') {
  const start = new Date(booking.start_date);
  const end   = new Date(booking.end_date);
  const days  = Math.max(1, Math.round((end - start) / (1000 * 60 * 60 * 24)));
  const total = booking.final_price || (car?.resale_price * days) || 0;
  ...
  const msg = [
    ...
    booking.client_passport ? `*Passeport :* ${booking.client_passport}` : null,
    ...
    ``,
    `_Demande en attente de confirmation. Réf: #${booking.id?.substring(0,8).toUpperCase()}_`,
