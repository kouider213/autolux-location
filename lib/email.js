...
// Source unique : ces valeurs viennent des Paramètres du site (table site_settings)
// et sont rafraîchies automatiquement → changer le numéro/adresse dans l'admin
// met à jour TOUS les emails. Valeurs par défaut tant que la DB n'a pas répondu.
let WA_NUM  = '32466311469';
let WA_DISP = '+32 466 31 14 69';
let ADDRESS = 'Rue Derbouz Draoua, Houari, Oran 31300, Algérie';
let MAPS    = 'https://share.google/N4itFBIAR9Z1JX8Aw';

async function refreshBrand() {
  try {
    const a = supabaseAdmin();
    if (!a) return;
    const { data } = await a.from('site_settings').select('whatsapp, address, maps_url').eq('id', 1).single();
    if (data?.whatsapp) { WA_NUM = String(data.whatsapp).replace(/\D/g, ''); WA_DISP = '+' + WA_NUM; }
    if (data?.address)  ADDRESS = data.address;
    if (data?.maps_url) MAPS = data.maps_url;
  } catch { /* garde les valeurs par défaut */ }
}
refreshBrand();                       // au démarrage
setInterval(refreshBrand, 600000);    // toutes les 10 min
...
// Relance automatique d'un lead (client intéressé qui n'a pas donné suite) — trilingue
export function leadFollowUpEmail({ client_name, subject, lang = 'fr' }) {
  const L = LG(lang);
  const title = T(L, { fr: 'Toujours intéressé ? 😊', ar: 'هل ما زلت مهتماً؟ 😊', en: 'Still interested? 😊' });
  const what = subject ? `<b>${subject}</b>` : T(L, { fr: 'ce qui vous intéressait', ar: 'ما كنت مهتماً به', en: 'what you were interested in' });
  const line = T(L, {
    fr: `Bonjour ${client_name || ''},<br/>Nous revenons vers vous au sujet de ${what}. C'est toujours disponible et notre équipe est là pour vous accompagner — sans engagement.`,
    ar: `مرحباً ${client_name || ''}،<br/>نعود إليك بخصوص ${what}. لا يزال متاحاً وفريقنا هنا لمساعدتك — دون أي التزام.`,
    en: `Hello ${client_name || ''},<br/>We're following up about ${what}. It's still available and our team is here to help — no commitment.`,
  });
  const cta = T(L, { fr: 'Nous contacter sur WhatsApp', ar: 'تواصل معنا عبر واتساب', en: 'Contact us on WhatsApp' });
  const html = wrap(title, `
    <p style="color:#555;font-size:14px;line-height:1.6">${line}</p>
    <p style="margin-top:16px"><a href="https://wa.me/${WA_NUM}" style="background:#25D366;color:#fff;text-decoration:none;padding:11px 20px;border-radius:8px;font-weight:700;font-size:14px;display:inline-block">💬 ${cta}</a></p>
  `, { lang: L, preheader: title });
  return { subject: T(L, { fr: 'Toujours intéressé ? — Fik Conciergerie', ar: 'هل ما زلت مهتماً؟ — Fik Conciergerie', en: 'Still interested? — Fik Conciergerie' }), html };
}
