...
export function bookingReceivedEmail(b) {
  const html = wrap('Votre demande de réservation est bien reçue', `
    <p style="color:#555;font-size:14px;line-height:1.6">Bonjour ${b.client_name},<br/>Nous vous remercions pour votre confiance. Votre demande de réservation a bien été reçue : notre équipe vérifie la disponibilité et reviendra vers vous très rapidement, par WhatsApp ou par email.</p>
...
export function bookingConfirmedEmail(b) {
  const html = wrap('Votre réservation est confirmée ✅', `
    <p style="color:#555;font-size:14px;line-height:1.6">Bonjour ${b.client_name},<br/>Nous avons le plaisir de vous confirmer votre réservation. Toute l'équipe Fik Conciergerie se réjouit de vous accueillir.</p>
    <table style="width:100%;border-collapse:collapse;margin-top:12px">
      ${row('Véhicule', b.car_name || '—')}
      ${row('Du', b.start_date)}${row('Au', b.end_date)}
...
// Changement de statut d'une réservation (générique, tous statuts)
const BOOKING_STATUS = {
  PENDING:   { label: 'En attente de confirmation', line: "Nous vous remercions pour votre demande de réservation. Notre équipe l'étudie avec la plus grande attention et reviendra vers vous dans les plus brefs délais." },
  ACCEPTED:  { label: 'Confirmée ✅', line: "Nous avons le plaisir de vous confirmer votre réservation. Toute l'équipe Fik Conciergerie se réjouit de vous accueillir et reste à votre entière disposition." },
  CONFIRMED: { label: 'Confirmée ✅', line: "Nous avons le plaisir de vous confirmer votre réservation. Toute l'équipe Fik Conciergerie se réjouit de vous accueillir et reste à votre entière disposition." },
  ACTIVE:    { label: 'En cours 🚗', line: "Votre location est désormais en cours. Nous vous souhaitons une excellente route et restons à votre disposition pour toute question." },
  COMPLETED: { label: 'Terminée', line: "Votre location est à présent terminée. Nous vous remercions chaleureusement de votre confiance — ce fut un réel plaisir de vous accompagner." },
  REJECTED:  { label: 'Indisponible à ces dates', line: "Nous sommes sincèrement navrés : nous ne sommes malheureusement pas disponibles sur ces dates. Notre équipe serait toutefois ravie de vous proposer d'autres options qui vous conviennent — n'hésitez pas à nous contacter." },
  CANCELLED: { label: 'Annulée', line: "Votre réservation a bien été annulée. Pour toute question, ou si vous souhaitez réserver à nouveau, notre équipe reste à votre entière disposition." },
};

export function bookingStatusEmail(b) {
  const st = BOOKING_STATUS[b.status] || BOOKING_STATUS.PENDING;
  const html = wrap('Mise à jour de votre réservation', `
    <p style="color:#555;font-size:14px;line-height:1.6">Bonjour ${b.client_name || ''},<br/>${st.line}</p>
    <div style="margin:16px 0;padding:14px 18px;background:#faf6e8;border:1px solid #e9b94944;border-radius:10px;text-align:center">
      <span style="color:#7a5c00;font-size:12px;text-transform:uppercase;letter-spacing:1px">Statut</span><br/>
      <span style="color:#111;font-size:20px;font-weight:800">${st.label}</span>
    </div>
    <table style="width:100%;border-collapse:collapse;margin-top:6px">
      ${b.car_name ? row('Véhicule', b.car_name) : ''}
      ${b.start_date ? row('Du', b.start_date) : ''}${b.end_date ? row('Au', b.end_date) : ''}
      ${b.total ? row('Total', money(b.total, b.currency)) : ''}
    </table>
    ${b.booking_id ? `<p style="margin-top:16px"><a href="https://fikconciergerie.com/suivi/${b.booking_id}" style="background:#e9b949;color:#1a1500;text-decoration:none;padding:11px 20px;border-radius:8px;font-weight:700;font-size:14px">Suivre ma réservation</a></p>` : ''}
  `, { preheader: `Statut : ${st.label}` });
  return { subject: `Réservation ${st.label} — Fik Conciergerie`, html };
}
