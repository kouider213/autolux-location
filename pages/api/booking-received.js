...
export default async function handler(req, res) {
  ...
  const { client_email, client_name, car_name, start_date, end_date, total, currency, booking_id } = req.body || {};
  if (!client_email || !/@/.test(client_email)) return res.status(200).json({ skipped: true });

  const { subject, html } = bookingReceivedEmail({ client_name, car_name, start_date, end_date, total, currency, booking_id });
