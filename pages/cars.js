          ...
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-14">
              ...
              <p className="text-white/40 mt-4 max-w-xl mx-auto">
                {cars.length} véhicule{cars.length > 1 ? 's' : ''} disponibles.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 justify-center mb-8">
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setFilter(cat)}
                  className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all duration-200 capitalize ${filter === cat ? 'bg-gold-500 text-noir-950 border-gold-500' : 'border-white/10 text-white/50 hover:border-gold-500/50 hover:text-white/80'}`}>
                  {cat}
                </button>
              ))}
            </div>
            <div className="flex items-center justify-center gap-4 mb-12">
              <span className="text-white/40 text-sm whitespace-nowrap">Prix max / jour :</span>
              <input type="range" min="15" max="200" value={maxPrice} onChange={e => setMaxPrice(Number(e.target.value))} className="w-32 accent-yellow-500" />
            ...
            {filtered.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filtered.map((car) => <CarCard key={car.id} car={car} router={router} />)}
              </div>
            )}
          </div>
        </div>
...
function CarCard({ car, router }) {
  const icons = { citadine: '🚗', berline: '🚙', SUV: '🚐', familiale: '🚌', utilitaire: '🚐', premium: '🏎️' };
  return (
    <div className="card-dark overflow-hidden hover:border-gold-500/30 hover:-translate-y-1 transition-all duration-300 group">
      <div className="bg-noir-800 h-48 flex items-center justify-center overflow-hidden relative cursor-pointer" onClick={() => router.push(`/cars/${car.id}`)}>
        {car.image_url ? (
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h3 className="text-white font-semibold text-lg leading-tight">{car.name}</h3>
          ...
          <div className="text-right flex-shrink-0 ml-2">
            <div className="text-gold-500 font-bold text-lg">{car.resale_price ? car.resale_price + ' €' : 'Sur demande'}</div>
            {car.resale_price && <div className="text-white/20 text-xs">/jour</div>}
          </div>
        </div>
        <Link href={`/reservation?car=${car.id}`} className="block w-full text-center bg-gold-500 text-noir-950 font-semibold py-2.5 rounded-xl hover:bg-gold-400 transition-colors duration-200 text-sm">
