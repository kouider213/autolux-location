...
export default function AdminCarsPage() {
  ...
  const handleSave = async () => {
    if (!form.name || !form.base_price || !form.resale_price) {
      toast.error('Nom, prix propriÃ©taire et prix revendeur sont obligatoires');
      return;
    }
    if (Number(form.resale_price) < Number(form.base_price)) {
      toast.error('Le prix revendeur doit Ãªtre supÃ©rieur ou Ã©gal au prix propriÃ©taire');
      return;
    }
    setSaving(true);
    ...
    toast.success(editCar ? 'VÃ©hicule modifiÃ©' : 'VÃ©hicule ajoutÃ©');
    setShowForm(false);
    loadCars();
  };
  ...
  const handleToggle = async (car) => {
    ...
    if (!error) {
      toast.success(car.available ? 'VÃ©hicule dÃ©sactivÃ©' : 'VÃ©hicule activÃ©');
      loadCars();
    }
  };

  const handleDelete = async (car) => {
    if (!confirm(`Supprimer ${car.name} ? Cette action est irrÃ©versible.`)) return;
    const { error } = await supabase.from('cars').delete().eq('id', car.id);
    if (error) toast.error('Erreur suppression');
    else { toast.success('VÃ©hicule supprimÃ©'); loadCars(); }
  };
  ...
  return (
    <>
      <Head><title>VÃ©hicules â Fik Conciergerie Admin</title></Head>
      <AdminLayout>
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="font-display text-3xl font-bold text-white">VÃ©hicules</h1>
              <p className="text-white/30 text-sm mt-1">{cars.length} vÃ©hicule(s) dans la flotte</p>
            </div>
                ...
                <div key={car.id} className={`card-dark overflow-hidden transition-all duration-200 ${!car.available ? 'opacity-50' : ''}`}>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex justify-between text-xs mb-3">
                      <div>
                        <span className="text-white/30">PropriÃ©taire : </span>
                        <span className="text-white/60">{car.base_price} â¬</span>
                      </div>
                      <div>
                        <span className="text-white/30">Revendeur : </span>
                        <span className="text-gold-500 font-semibold">{car.resale_price} â¬</span>
                      </div>
                    </div>
                    <div className="flex gap-1.5">
                      ...
                      <button onClick={() => handleToggle(car)} className={`flex-1 text-xs py-1.5 rounded-lg transition-colors ${
                        ...
                        {car.available ? 'â¸ DÃ©sact.' : 'â¶ Activer'}
                      </button>
          ...
          <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
            <div className="card-dark w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="p-6 border-b border-white/5 flex justify-between items-center">
                <h2 className="text-white font-semibold text-lg">{editCar ? 'Modifier le vÃ©hicule' : 'Ajouter un vÃ©hicule'}</h2>
                <button onClick={() => setShowForm(false)} className="text-white/30 hover:text-white text-xl">Ã</button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="label-dark">Nom du vÃ©hicule *</label>
                  <input value={form.name} onChange={up('name')} placeholder="Ex: Clio 5" className="input-dark" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label-dark">Prix propriÃ©taire (â¬/j) *</label>
                    <input type="number" value={form.base_price} onChange={up('base_price')} placeholder="Ex: 37" className="input-dark" />
                  </div>
                  <div>
                    <label className="label-dark">Prix revendeur (â¬/j) *</label>
                ...
                {form.base_price && form.resale_price && (
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-4 py-2 text-sm">
                    ...
                    <span className="text-emerald-400 font-bold">+{(form.resale_price - form.base_price).toFixed(0)} â¬</span>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label-dark">CatÃ©gorie</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={form.fuel} onChange={up('fuel')} className="input-dark">
                      {['essence', 'diesel', 'hybride', 'Ã©lectrique'].map(f => <option key={f} value={f}>{f}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="label-dark">BoÃ®te</label>
                ...
                <button onClick={handleSave} disabled={saving} className="btn-gold w-full py-3 disabled:opacity-50">
                  {saving ? 'Sauvegarde...' : editCar ? 'Enregistrer les modifications' : 'Ajouter le vÃ©hicule'}
                </button>
              </div>
            </div>
          </div>
