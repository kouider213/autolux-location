        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Rechercher un client, véhicule..." className="input-dark max-w-xs" />
            <div className="flex flex-wrap gap-2">
              {STATUS_FILTERS.map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                    filter === f ? 'bg-gold-500 text-noir-950 border-gold-500' : 'border-white/10 text-white/40 hover:border-gold-500/40'
                  }`}>
                  {f === 'Tous' ? 'Tous' : f === 'PENDING' ? 'En attente' : f === 'ACCEPTED' ? 'Acceptées' : 'Refusées'}
                  {f !== 'Tous' && <span className="ml-1 opacity-60">({bookings.filter(b => b.status === f).length})</span>}
                </button>
              ))}
            </div>
          </div>
                ...
                <table className="w-full min-w-[600px]">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filtered.map((b) => {
                      ...
                      return (
                        <tr key={b.id} className="hover:bg-white/2 transition-colors cursor-pointer" onClick={() => openDetail(b)}>
                              ...
                              {b.status === 'PENDING' && (
                                <>
                                  <button onClick={() => updateStatus(b.id, 'ACCEPTED')} disabled={actionLoading}
                                    className="bg-emerald-500/20 hover:bg-emerald-500/40 text-emerald-400 text-xs px-2.5 py-1.5 rounded-lg transition-colors disabled:opacity-50" title="Accepter">✓</button>
                                  <button onClick={() => updateStatus(b.id, 'REJECTED')} disabled={actionLoading}
                                    className="bg-red-500/20 hover:bg-red-500/40 text-red-400 text-xs px-2.5 py-1.5 rounded-lg transition-colors disabled:opacity-50" title="Refuser">✕</button>
                                </>
                              )}
                              <button onClick={() => handleWhatsApp(b)}
                                className="bg-green-600/20 hover:bg-green-600/40 text-green-400 text-xs px-2.5 py-1.5 rounded-lg transition-colors" title="WhatsApp">💬</button>
                              {b.status === 'ACCEPTED' && (
                                <button onClick={() => handlePDF(b)}
                                  className="bg-blue-500/20 hover:bg-blue-500/40 text-blue-400 text-xs px-2.5 py-1.5 rounded-lg transition-colors" title="Télécharger PDF">📄</button>
                              )}
                            </div>
                          </td>
                        </tr>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1 bg-noir-950 border border-gold-500/40 rounded-lg px-3 py-1.5 flex-1">
                            <input type="number" value={editPrice} onChange={e => setEditPrice(e.target.value)}
                              min="0" step="1" className="bg-transparent text-white text-sm w-full outline-none"
                              placeholder={selected.final_price || '0'} />
