...
function BenefitsCarousel({ benefits }) {
  ...
  useEffect(() => {
    if (paused) return;
    const t = setInterval(() => setIdx(i => (i + 1) % benefits.length), 3000);
  ...
  return (
    <div className="relative w-full"
      onTouchStart={e => { setTxStart(e.touches[0].clientX); setPaused(true); }}
      onTouchEnd={e => {
        const diff = txStart - e.changedTouches[0].clientX;
        if (Math.abs(diff) > 50) setIdx(i => diff > 0 ? (i+1)%benefits.length : (i-1+benefits.length)%benefits.length);
        ...
        <motion.div key={idx}
          ...
          <div className="flex items-center justify-center px-4 py-10 md:py-14">
            <div className="max-w-lg text-center">
              <div className="w-16 h-16 bg-gold-500/[0.08] border border-gold-500/25 rounded-2xl flex items-center justify-center mx-auto mb-6">
                <slide.icon size={26} className="text-gold-400" />
              </div>
              <p className="text-white/40 text-xs tracking-[0.3em] uppercase font-body mb-3">{slide.num} / 06</p>
              <h2 className="font-display font-black text-white mb-4 leading-tight" style={{ fontSize:'clamp(28px,5vw,52px)' }}>
                {slide.title}
              </h2>
              <p className="text-white/60 text-base font-body leading-relaxed">{slide.desc}</p>
            </div>
          </div>
        </motion.div>
      </AnimatePresence>
      <div className="flex justify-center gap-2 pb-4">
        {benefits.map((_, i) => (
          <button key={i}
            ...
            className={`rounded-full transition-all duration-300 ${i === idx ? 'w-8 h-2 bg-gold-500' : 'w-2 h-2 bg-white/20'}`} />
        ))}
      </div>
      <p className="text-center text-white/20 text-xs pb-4 md:hidden">Glissez pour naviguer</p>
    </div>
  );
}
...
function CarCarousel({ cars }) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden"
      ...
      onTouchStart={e => setTxStart(e.touches[0].clientX)}
      onTouchEnd={e => { const d = txStart - e.changedTouches[0].clientX; if (Math.abs(d) > 50) d > 0 ? next() : prev(); }}>

      <AnimatePresence mode="wait" custom={dir}>
        <motion.div key={idx} custom={dir}
          variants={{ enter: d => ({ x: d > 0 ? '100%' : '-100%', opacity:0 }), center: { x:0, opacity:1 }, exit: d => ({ x: d > 0 ? '-60%' : '60%', opacity:0 }) }}
          initial="enter" animate="center" exit="exit"
          transition={{ duration:0.5, ease:[0.16,1,0.3,1] }}
          className="absolute inset-0">
          {car.image_url
            ? <img src={car.image_url} alt={car.name} className="w-full h-full object-cover object-center" loading="lazy" />
            : <div className="w-full h-full bg-[#141414] flex items-center justify-center"><Car size={80} className="text-white/[0.05]" /></div>
          }
          ...
          <motion.div className="absolute bottom-0 left-0 right-0 p-5 md:p-10"
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-white/25 text-xs font-body">{String(idx+1).padStart(2,'0')} / {String(total).padStart(2,'0')}</span>
            </div>
            <h3 className="font-display font-black text-white leading-[0.9] mb-3" style={{ fontSize:'clamp(30px,6vw,64px)' }}>{car.name}</h3>
            <div className="flex items-center gap-3 mb-5 flex-wrap">
              <span className="flex items-center gap-1.5 text-white/50 text-sm font-body"><Fuel size={12} className="text-gold-500/70" />{car.fuel}</span>
              <span className="text-white/20">·</span>
              <span className="flex items-center gap-1.5 text-white/50 text-sm font-body"><Users size={12} className="text-gold-500/70" />{car.seats}p</span>
              {car.transmission && <><span className="text-white/20">·</span><span className="text-white/50 text-sm capitalize font-body">{car.transmission}</span></>}
            </div>
            <div className="flex items-end justify-between gap-4">
              <div>
                <span className="text-white/30 text-xs tracking-widest uppercase font-body block mb-1">À partir de</span>
                <span className="font-display font-black text-gold-gradient leading-none" style={{ fontSize:'clamp(28px,4vw,48px)' }}>{car.resale_price}€</span>
                <span className="text-white/30 text-sm font-body ml-1">/jour</span>
              </div>
              <div className="flex gap-2">
                <Link href={`/cars/${car.id}`} className="btn-outline text-sm py-2.5 px-4 whitespace-nowrap">Détails</Link>
                <Link href={`/reservation?car=${car.id}`} className="btn-gold text-sm py-2.5 px-5 whitespace-nowrap"><CalendarCheck size={14} />Réserver</Link>
              </div>
            </div>
          </motion.div>
        </motion.div>
      ...
      <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-black/60 border border-white/10 rounded-full flex items-center justify-center text-white hover:bg-gold-500 hover:text-noir-950 transition-all">
        <ChevronLeft size={18} />
      </button>
      <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-black/60 border border-white/10 rounded-full flex items-center justify-center text-white hover:bg-gold-500 hover:text-noir-950 transition-all">
        <ChevronRight size={18} />
      </button>
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-20">
        {cars.map((_, i) => (
          <button key={i} onClick={() => goTo(i)} className={`rounded-full transition-all duration-300 ${i === idx ? 'w-5 h-1.5 bg-gold-500' : 'w-1.5 h-1.5 bg-white/30'}`} />
        ))}
      </div>
    </div>
  );
...
export default function Home({ cars: initialCars, reviews: initialReviews }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (c.data && c.data.length > 0) setCars(c.data);
      if (r.data && r.data.length > 0) setReviews(r.data);
    }).catch(() => {});
  }, []);

  const heroCar = cars.find(c => c.image_url && c.name?.toLowerCase().includes('mercedes'))
    ...
    <>
      ...
      <div className="grain bg-[#080808] overflow-x-hidden">
        ...
        <section className="relative overflow-hidden" style={{ minHeight:'100svh' }}>
          ...
          {/* Content */}
          <div className="relative z-10 flex flex-col" style={{ minHeight:'100svh', padding:'80px 20px 32px' }}>
            {/* Brand */}
            <div className="flex flex-col items-center gap-2 mt-4 mb-auto">
              <div className="w-14 h-14 bg-gradient-to-br from-gold-400 to-gold-600 rounded-2xl flex items-center justify-center shadow-[0_0_40px_rgba(226,182,20,0.5)] animate-float">
                <span className="font-black text-noir-950 text-xl">FK</span>
              </div>
              <p className="font-display text-lg font-bold text-white">Fik Conciergerie</p>
              <p className="text-white/35 text-[10px] tracking-[0.25em] uppercase font-body">Location Premium · Oran</p>
            </div>

            {/* Text bottom */}
            <div className="mt-auto">
              <h1 className="font-display font-black text-white leading-[0.88] mb-1" style={{ fontSize:'clamp(44px,10vw,108px)' }}>La Route,</h1>
              <h1 className="font-display font-black text-gold-gradient italic leading-[0.88] mb-5" style={{ fontSize:'clamp(44px,10vw,108px)' }}>Votre Style</h1>
              <p className="text-white/65 text-base font-body mb-1">Location de véhicules premium à Oran.</p>
              <p className="text-white/35 text-sm font-body mb-6">Sans caution · Réservation rapide · 7j/7</p>

              <div className="flex gap-3 flex-wrap mb-6">
                <Link href="/reservation" className="btn-gold px-6 py-3 text-sm"><CalendarCheck size={16} />Réserver</Link>
                <Link href="/cars" className="btn-outline px-5 py-3 text-sm"><Car size={14} />La flotte</Link>
              </div>
          ...
          <div className="relative z-10 max-w-5xl mx-auto">
            <div className="mb-8">
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-3">
                ...
                <Link href="/cars" className="btn-outline text-sm py-2.5 self-start whitespace-nowrap">Tous <ArrowRight size={13} /></Link>
              </div>
            </div>
            {cars.length > 0 && <CarCarousel cars={cars.filter(c => c.available !== false)} />}
          </div>
        </section>
        ...
        <section className="relative py-14 md:py-20 px-5 overflow-hidden bg-[#050505]">
          ...
          <div className="relative z-10 max-w-5xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="font-display text-3xl md:text-5xl font-bold text-white mt-3">Confiance & <span className="text-gold-gradient italic">Excellence</span></h2>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={i} className="text-center">
                  <div className="font-display font-black text-gold-gradient leading-none mb-2" style={{ fontSize:'clamp(38px,6vw,68px)' }}>
                    <AnimCounter to={s.val} suffix={s.suffix} />
                  </div>
                  <div className="text-white font-semibold text-sm font-body mb-1">{s.label}</div>
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
              <div className="text-center mb-12">
                ...
                <div className="flex items-center justify-center gap-2 mt-3">
                  ...
                  <span className="text-white/35 text-sm font-body ml-1">{avgRating}/5</span>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {reviews.slice(0,3).map((review, i) => (
                  <div key={review.id} className="card-dark p-6 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-white/[0.06]">
                      <div className="w-9 h-9 bg-gold-500/10 border border-gold-500/20 rounded-full flex items-center justify-center flex-shrink-0">
        ...
        {/* ══ CTA ══ */}
        <section className="relative py-20 px-5 overflow-hidden bg-[#050505]">
          <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-gold-500/20 to-transparent" />
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center">
            ...
            <p className="text-white/35 text-base mb-10 font-body">Réservez dès maintenant votre véhicule à Oran.<br />Simple, rapide, sans caution.</p>
...
export async function getStaticProps() {
  try {
    if (!supabase) return { props: { cars: [], reviews: [] }, revalidate: 60 };
    ...
    return { props: { cars: [], reviews: [] }, revalidate: 60 };
  }
}
