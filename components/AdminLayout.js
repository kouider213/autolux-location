...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-noir-950 flex">
      ...
      {/* ── SIDEBAR ── */}
      <aside className={`
        fixed md:sticky top-0 left-0 h-screen w-64 bg-noir-900 border-r border-white/5
      ...
      `}>
        ...
        <div className="p-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold text-sm">AutoLux</p>
        ...
        <div className="p-4 border-t border-white/5 space-y-2">
          <Link
            href="/"
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-white/30 hover:text-white/60 transition-colors"
          >
            <span>🌐</span> Voir le site
          </Link>
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-red-400/60 hover:text-red-400 hover:bg-red-500/10 transition-all duration-200"
          >
